...
def check_bounds_under_attacks(data, labels, dataset_name, k=1000, nq=50):
    ...
    ub_column_names = ['delta', 'perturb_farther', 'lid_b', 'fb_delta_x', 'fb_delta_y', 'y', 'x', 'cos_psi', 'eta', 'ub']
    ub_df = pd.DataFrame(columns=ub_column_names)

    lb_column_names = ['delta', 'perturb_farther', 'lid_b', 'fb_delta_x', 'fb_delta_y', 'y', 'x', 'cos_psi', 'eta', 'lb']
    lb_df = pd.DataFrame(columns=lb_column_names)

    original_ub_column_names = ['delta', 'perturb_farther', 'lid_b', 'fb_delta_x', 'fb_delta_y', 'y', 'x', 'cos_psi', 'eta', 'original_ub']
    original_ub_df = pd.DataFrame(columns=original_ub_column_names)

    original_lb_column_names = ['delta', 'perturb_farther', 'lid_b', 'fb_delta_x', 'fb_delta_y', 'y', 'x', 'cos_psi', 'eta', 'original_lb']
    ...
    if dataset_name == 'mnist':
        bandwidth = 0.2212216291070449
    elif dataset_name == 'cifar':
        bandwidth = 1.2689610031679222
    ...
    for a_index in idx:
        ...
        for delta in np.arange(delta_start, delta_stop, delta_increment):
            ...
            if bandwidth is None:
                ...
                grid.fit(distances_from_b.reshape(-1, 1))
                kde = grid.best_estimator_
                bandwidth = kde.bandwidth
                print(f'bandwidth - {kde.bandwidth}')
            else:
                kde = KernelDensity(bandwidth=bandwidth, kernel='gaussian')
                distances_from_b = distances_from_b[np.isfinite(distances_from_b)]
                try:
                    kde.fit(distances_from_b.reshape(-1, 1))
                except ValueError as e:
                    continue

            def normal_distribution_function(x_value):
                value = kde.score_samples(np.array(x_value).reshape(-1, 1))
                return np.exp(value)

            fb_delta_x, error = quad(normal_distribution_function, 0, delta_x)
            fb_delta_y, error = quad(normal_distribution_function, 0, y)
            lid_b = get_lids(b, k=k, batch=data)[0]

            numerator = np.log(fb_delta_y / fb_delta_x)
            denominator = 1 / delta
            ...
            for eta in eta_range:
                try:
                    ...
                    if (1 - (1 / delta)) < eta < ((1 / delta) - 1):
                        ...
                        ub_dict.append(
                            dict(zip(ub_column_names,
                                     [delta, perturb_farther, lid_b, fb_delta_x, fb_delta_y, y, x, b_direction_angle_cos, eta, ub])))
                except Exception:
                    pass
            ...
            for eta in eta_range:
                try:
                    ...
                    if (1 - (1 / delta)) < eta < ((1 / delta) - 1):
                        ...
                        lb_dict.append(
                            dict(zip(lb_column_names,
                                     [delta, perturb_farther, lid_b, fb_delta_x, fb_delta_y, y, x, b_direction_angle_cos, eta, lb])))
                except Exception:
                    pass
                ...
                try:
                    ...
                    original_ub_dict.append(
                        dict(zip(original_ub_column_names,
                                 [delta, perturb_farther, lid_b, fb_delta_x, fb_delta_y, y, x, b_direction_angle_cos, eta, original_ub])))
                except Exception:
                    pass
                ...
                try:
                    ...
                    original_lb_dict.append(
                        dict(zip(original_lb_column_names,
                                 [delta, perturb_farther, lid_b, fb_delta_x, fb_delta_y, y, x, b_direction_angle_cos, eta, original_lb])))
                except Exception:
                    pass
...
if dataset == 'mnist':
    ...
    indices = np.random.permutation(x_train.shape[0])[:n]
